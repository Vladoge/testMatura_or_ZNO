...
- (void)goToCoachMarkIndexed:(NSUInteger)index {
    ...
    if (self.enableContinueLabel) {
        if (markIndex == 0) {
            ...
            lblContinue.text = @"Нажмите чтобы продолжить";
    ...
    if (self.enableSkipButton) {
        ...
        [ _btnSkipCoach setTitle:@"Пропустить" forState:UIControlStateNormal];
         _btnSkipCoach.titleLabel.font = [UIFont boldSystemFontOfSize:13.0f];
         _btnSkipCoach.alpha = 0.0f;
         _btnSkipCoach.tintColor = [UIColor whiteColor];
        [self addSubview: _btnSkipCoach];
        [UIView animateWithDuration:0.3f delay:1.0f options:0 animations:^{
             _btnSkipCoach.alpha = 1.0f;
        } completion:nil];
    }
}
